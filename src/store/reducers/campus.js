...
const campus = (state = initialState, action) => {  // Use "initialState" as default Initial State
  switch (action.type) {
    ...
    case DELETE_CAMPUS:
      // Check if the deleted campus is the same as the current one in the state.
      // If it is, return initial state, effectively clearing the current campus.
      if (state.id === action.payload) {
        return initialState;
      }
      // If the deleted campus is not the current one, return the state unchanged.
      return state;
    default:
